...
class MultiDiscreteDist(spaces.MultiDiscrete):
    ...
    def __call__(self, prob: th.Tensor, mask: th.Tensor = None) -> MaskedCategorical:
        ...
        Create a `MaskedCategorical` distribution by reshaping the input probabilities, applying an optional mask, and combining with an internal mask.

        :param prob: A tensor containing probabilities to be reshaped and used in constructing the distribution.
        :type prob: th.Tensor
        :param mask: An optional boolean tensor for masking specific probabilities before creating the distribution. Defaults to None.
        :type mask: th.Tensor, optional
        :return: A `MaskedCategorical` distribution object created with reshaped probabilities and combined masking information.
